...
class MusicController {
    ...
    async addMusic(req, res) {
        const { MusicName, Img, GenreID, GenreName } = req.body;
        console.log('Nội dung yêu cầu:', req.body);  // Ghi log nội dung yêu cầu

        if (!MusicName || !Img || !GenreID || !GenreName) {
        ...
        try {
            ...
            const newMusic = { id: uuidv4(), MusicName, Img, GenreID, GenreName };
    ...
    async getListAll(req, res) {
        try {
            const Musics = this.loadMusicData();
            res.json({ Musics, message: 'Thành công' });
        } catch (error) {
